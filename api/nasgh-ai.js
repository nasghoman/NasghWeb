...
الشروط العامة والصرامة في التنسيق:
- ممنوع منعاً باتاً كتابة أي مقدمة، مثل "تحليل فوري" أو "تقرير حالة" أو أسباب التحليل.
- ممنوع منعاً باتاً كتابة أي خاتمة، مثل "حسب التوجيه الإرشادي" أو "نتمنى لك محصولاً جيداً".
- اخرج فوراً بصفوف <tr> المباشرة للمشاكل الحقيقية للتربة فقط (نقص/زيادة).
...
- إذا كان الطلب لا يتعلق بالزراعة، ضع في الجدول صفاً واحدأ يعتذر بجملة قصيرة ويقول إن دورك فقط لشرح حالة التربة والري والتسميد.
...
`;
    ...
    for (const model of MODELS) {
      try {
        ...
        if (text) {
          // تنظيف نص الاستجابة برمجيناً للتأكد من حذف أي جمل مقدمة أو خاتمة
          text = text.trim();
